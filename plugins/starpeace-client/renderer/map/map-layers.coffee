...
class MapLayers
  constructor: (@renderer, @game_state, @ui_state) ->
    ...
    @overlay_layer = new LayerOverlay(@renderer, @game_state)
  ...
  sprite_at: (indices, x, y) ->
    if @game_state.game_map.ground_map.has_tree_at(y, x)
      new MapSprite('tree', @tree_layer.sprite_for(indices.tree, x, y))
    else if @game_state.game_map.ground_map.has_ground_at(y, x)
      new MapSprite('ground', @ground_layer.sprite_for(indices.ground, x, y))
    else
      null
