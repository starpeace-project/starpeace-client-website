...
export default class SandboxInventions
  ...
  get_bookmarks: (corporation_id) ->
    return _.cloneDeep(@sandbox.sandbox_data?.corporation.bookmarksByCorporationId[corporation_id]?.bookmarks || [])
  ...
  create_bookmark: (corporation_id, parameters) ->
    ...
    @sandbox.sandbox_data.corporation.bookmarksByCorporationId[corporation_id] = { bookmarks: [] } unless @sandbox.sandbox_data.corporation.bookmarksByCorporationId[corporation_id]?
    order = _.filter(@sandbox.sandbox_data.corporation.bookmarksByCorporationId[corporation_id].bookmarks, (bookmark) -> bookmark.parentId == parameters.parentId).length
    ...
    @sandbox.sandbox_data.corporation.bookmarksByCorporationId[corporation_id].bookmarks.push item
  ...
  update_bookmarks: (corporation_id, deltas) ->
    ...
    updated = []
    for delta in deltas
      existing = _.find(@sandbox.sandbox_data.corporation.bookmarksByCorporationId[corporation_id].bookmarks, (bookmark) -> bookmark.id == delta.id)
      if existing?
