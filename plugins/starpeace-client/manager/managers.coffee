...
import StringManager from '~/plugins/starpeace-client/manager/string-manager.coffee'
...
export default class Managers
  constructor: (@event_listener, @game_state, @ui_state) ->
    ...
    @string_manager = new StringManager(@asset_manager, @event_listener, @game_state)
  ...
  has_assets: () ->
    @game_state.current_planet? && @building_manager.has_assets() && @concrete_manager.has_assets() && @effect_manager.has_assets() && @event_manager.has_assets() &&
      @overlay_manager.has_assets() && @planetary_manager.has_assets(@game_state.current_planet) && @plane_manager.has_assets() && @road_manager.has_assets() && @string_manager.has_assets()
  ...
  queue_asset_load: () ->
    ...
    @string_manager.queue_asset_load()
